...
% covid with NaN
DataMTrf = DataM;
idx = (TransfM(:,1) == 2) | (TransfM(:,1) == 4);  % Solo 2 e 4
DataMTrf(:, idx) = 100 * DataMTrf(:, idx);
...
% covid with NaN
DataQTrf = DataQ;
idx = (TransfQ(:,1) == 2) | (TransfQ(:,1) == 4);
DataQTrf(:, idx) = 100 * DataQTrf(:, idx);
...
Mx = nanmean(Data);
Wx = nanstd(Data);
xNaN = (Data - repmat(Mx, T, 1)) ./ repmat(Wx, T, 1);

% Scree Plot - PCA Eigenvalues

Sigma = cov(xNaN, 'omitrows'); % Matrice di covarianza
[V, D] = eig(Sigma);         % Autovettori (V) e autovalori (D)
[eigenvalues, idx] = sort(diag(D), 'descend');
V = V(:, idx);               % Riordina autovettori
scores = xNaN * V;              % Proiezione dei dati
% Plot dei primi due scores
figure;
...
[coeff, score, latent] = pca(xNaN, 'Rows', 'complete');
...
% Bai & Ng (2002) Information Criterion (ICp1)

max_factors = 10;
SSRs = zeros(max_factors, 1);
ICp1 = zeros(max_factors, 1);

for r = 1:max_factors
    [coeff_r, score_r, ~] = pca(xNaN, 'NumComponents', r, 'Rows', 'complete');
    Data_hat = score_r * coeff_r';
    
    SSR = sum((xNaN - Data_hat).^2, 'all', 'omitnan') / (T * N);
    SSRs(r) = SSR;
    
    ICp1(r) = log(SSR) + r * (log(T * N) / (T * N));
end

figure;
plot(1:max_factors, ICp1, '-o');
xlabel('Number of Factors');
ylabel('ICp1 Criterion');
title('Factor Selection - ICp1 (Bai & Ng, 2002)');
grid on;

% Cumulative Explained Variance

Sigma = cov(xNaN, 'omitrows');
[eigenvectors, eigenvalues_matrix] = eig(Sigma);
...
figure;
plot(1:length(cumulative_variance), cumulative_variance, '-o', 'LineWidth', 2);
yline(0.8, '--r', '80% Threshold');
xlabel('Number of Principal Components');
ylabel('Cumulative Explained Variance');
title('Cumulative Explained Variance by Principal Components');
grid on;
xlim([1, length(cumulative_variance)]);
ylim([0, 1]);
