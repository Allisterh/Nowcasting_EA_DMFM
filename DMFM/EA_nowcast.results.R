...
DateQQ_dt <- as.Date(as.numeric(mat_data$DateQQ), origin = "0000-01-01")
...
gdp_array    <- gdp_array[3:32, , ea_index]
...
df_rmsfe_dmfm$Period <- factor(
  ifelse(df_rmsfe_dmfm$Date < P$covid_start, "Pre-COVID",
...
df_rmsfe_dfm$Period <- factor(
  ifelse(df_rmsfe_dfm$Date < P$covid_start, "Pre-COVID",
...
# Etichette trimestrali e date reali da 2017Q4
n_quarters <- length(unique(df_all$Quarter))
quarter_dates <- seq(as.Date("2017-10-01"), by = "3 months", length.out = n_quarters)
quarter_labels <- paste0(format(quarter_dates, "%Y"), "Q", ceiling(as.numeric(format(quarter_dates, "%m")) / 3))

# Colonna Date coerente
df_all$Date <- rep(quarter_dates, times = nrow(df_all) / n_quarters)

# Periodo COVID come intervallo di date
covid_start_date <- as.Date("2020-03-01")  # 2020Q1
covid_end_date   <- as.Date("2021-06-30")  # 2021Q2
df_rect <- data.frame(
...
ggplot() +
  ...
  # Serie temporali
  geom_line(data = df_all, aes(x = Date, y = Value, color = Month, group = Month), size = 1.2) +
  ...
  # Etichette asse X in formato "YYYYQn"
  scale_x_date(
    breaks = quarter_dates,
    labels = paste0(format(quarter_dates, "%Y"), "Q", ceiling(as.numeric(format(quarter_dates, "%m")) / 3))
  ) +
...
ggplot() +
  ...
  # True GDP points and line
  geom_point(data = true_gdp_series, aes(x = TimeIndex, y = GDP),
             color = "black", size = 2.5, shape = 16) +
  geom_line(data = true_gdp_series, aes(x = TimeIndex, y = GDP),
  ...
  # Labels
  labs(
    title = "True GDP vs Nowcast Updates",
    subtitle = "Comparison by Aggregated Month of Forecast",
    x = "Nowcast Month", y = "GDP Growth (%)",
    color = NULL
  ) +
...
# Crea 30 etichette trimestrali da 2010Q1 a 2017Q4 (o da 2015Q1 a 2017Q4 se sono solo 12 trimestri)
quarters <- seq(as.Date("2017-04-01"), by = "3 months", length.out = length(unique(true_gdp_series$TimeIndex)))
quarter_labels <- paste0(format(quarters, "%Y"), " Q", ceiling(as.numeric(format(quarters, "%m")) / 3))

scale_x_continuous(
  breaks = true_gdp_series$TimeIndex,
  labels = quarter_labels
)

# Crea le etichette
quarters <- seq(as.Date("2017-10-01"), by = "3 months", length.out = length(unique(true_gdp_series$TimeIndex)))
quarter_labels <- paste0(format(quarters, "%Y"), " Q", ceiling(as.numeric(format(quarters, "%m")) / 3))
...
ggplot() +
  ...
  theme(
    ...
    panel.grid.minor = element_blank(),
    plot.margin = margin(10, 10, 10, 10),
    axis.text.x = element_text(angle = 45, hjust = 1)  # opzionale, per migliorare leggibilità
  )

# Join tra previsioni e valori reali
df_rmsfe <- df_plot_long %>%
  dplyr::left_join(true_gdp_series, by = "TimeIndex") %>%
  dplyr::mutate(squared_error = (Nowcast - GDP)^2)

# Calcolo RMSFE per ciascun modello
rmsfe_by_model <- df_rmsfe %>%
  dplyr::group_by(Model) %>%
  dplyr::summarise(RMSFE = sqrt(mean(squared_error, na.rm = TRUE)))

print(rmsfe_by_model)

rmsfe_pre_post <- df_rmsfe %>%
  dplyr::mutate(period = ifelse(TimeIndex < covid_start_q, "Pre-COVID", "Post-COVID")) %>%
  dplyr::group_by(Model, period) %>%
  dplyr::summarise(RMSFE = sqrt(mean(squared_error, na.rm = TRUE)))

print(rmsfe_pre_post)

rmsfe_pre_post <- df_rmsfe %>%
  dplyr::mutate(period = dplyr::case_when(
    TimeIndex < covid_start_q         ~ "Pre-COVID",
    TimeIndex >= covid_start_q & TimeIndex <= covid_end_q ~ "During-COVID",
    TimeIndex > covid_end_q           ~ "Post-COVID"
  )) %>%
  dplyr::group_by(Model, period) %>%
  dplyr::summarise(RMSFE = sqrt(mean(squared_error, na.rm = TRUE)))
...
# Given the identification condition factors are interpretable up to the sign. 
# so for interpretability just invert it 
...
plot_factor_loadings_heatmap <- function(R, C, var_names, country_names, factor_idx = 1) {
  ...
  # Estrai vettori di loadings per il fattore scelto
  r_f <- R[, factor_idx]  # loading delle variabili
  c_f <- C[, factor_idx]  # loading dei paesi
  
  # Outer product: variabili (righe) × paesi (colonne)
  L_mat <- outer(r_f, c_f)
  rownames(L_mat) <- var_names
  colnames(L_mat) <- country_names
  
  # Converti in formato lungo
  L_df <- as.data.frame(L_mat) %>%
    mutate(Variable = rownames(L_mat)) %>%
    pivot_longer(-Variable, names_to = "Country", values_to = "Loading")
  
  # Heatmap
  p <- ggplot(L_df, aes(x = Country, y = Variable, fill = Loading)) +
    geom_tile(color = "white") +
    scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
    labs(title = paste("Factor", factor_idx, "Loadings by Variable and Country"),
         x = NULL, y = NULL) +
...
R <- inputs$R      # dimensione: V × r
C <- inputs$C      # dimensione: N × r
var_names <-  colnames(tensor$Y) # oppure dati tuoi
country_names <- colnames(tensor$Y[1,,])

plot_factor_loadings_heatmap(R, C, var_names, country_names, factor_idx = 1)
...
plot_factor_loadings_heatmap <- function(R, C, var_names, country_names, factor_idx = 1) {
  ...
  # Estrai vettori di loadings per il fattore scelto
  r_f <- R[, factor_idx]  # loadings variabili
  c_f <- C[, factor_idx]  # loadings paesi
...
country_names <- colnames(tensor$Y[1,,])  # nomi dei paesi
...
# 1. Plotta la serie temporale dei fattori
plot_factors_tensor <- function(fs) {
  fs_df <- as.data.frame(fs)
  fs_df$Time <- 1:nrow(fs_df)
  fs_long <- pivot_longer(fs_df, -Time, names_to = "Factor", values_to = "Value")
  
  ggplot(fs_long, aes(x = Time, y = Value, color = Factor)) +
    geom_line(linewidth = 1) +
    labs(title = "Time Series of Latent Factors",
         x = "Time", y = "Factor Value") +
    theme_minimal(base_size = 13) +
    theme(legend.position = "bottom", plot.title = element_text(hjust = 0.5))
}

# 2. Grafico: Loadings delle variabili (riga)
plot_variable_loadings <- function(R, var_names, factor_idx) {
  r_f <- R[, factor_idx]
  df <- data.frame(Variable = var_names, Loading = r_f)
  
  ggplot(df, aes(x = reorder(Variable, Loading), y = Loading, fill = Loading)) +
    geom_col() +
    coord_flip() +
    scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
    labs(title = paste("Variable Loadings for Factor", factor_idx),
         x = NULL, y = "Loading") +
    theme_minimal(base_size = 13) +
    theme(plot.title = element_text(hjust = 0.5), legend.position = "none")
}

# 3. Grafico: Loadings dei paesi (colonna)
plot_country_loadings <- function(C, country_names, factor_idx) {
  c_f <- C[, factor_idx]
  df <- data.frame(Country = country_names, Loading = c_f)
  
  ggplot(df, aes(x = reorder(Country, Loading), y = Loading, fill = Loading)) +
    geom_col() +
    coord_flip() +
    scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
...
l.R_str <- inputs$R  # loadings variabili (V x r)
l.C_str <- inputs$C  # loadings paesi (N x r)
...
library(knitr)
library(kableExtra)
rownames_R <- rownames(l.R_str)
rownames_C <- rownames(l.C_str)

# Valori negati
row_vals <- -l.R_str
col_vals <- -l.C_str

# Unione in una matrice
combined <- rbind(
  `F_row_1` = c(row_vals, rep(NA, length(col_vals))),  # padding con NA per allineamento
  `F_col_1` = c(rep(NA, length(row_vals)), col_vals)
)

# Nomi colonne
colnames(combined) <- c(rownames_R, rownames_C)

# Tabella LaTeX
kable(combined, format = "latex", digits = 3, booktabs = TRUE,
      caption = "Negated values of $l.R\\_str$ and $l.C\\_str$") %>%
  kable_styling(latex_options = c("scale_down", "hold_position", "striped"))


var_names <- colnames(tensor$Y[1,,])        # V variabili
country_names <- colnames(tensor$Y)# N paesi

# 1. Serie temporali dei fattori
plot_factors_tensor(inputs$fs)

# 2. Plot dei loadings separati per fattore
for (k in 1:ncol(l.R_str)) {
  print(plot_variable_loadings(l.R_str, country_names, k))
}

for (k in 1:ncol(l.C_str)) {
  print(plot_country_loadings(l.C_str, var_names, k))
}
...
# ==============================================================================
# STEP 5 — GRAFICO: GDP OSSERVATO vs STIMA SMOOTHED DI TUTTI I PAESI
# ==============================================================================


# Estrai indici di GDP
GDP_index <- which(dimnames(Y)[[3]] == GDP_varname)
Dates     <- dimnames(Y)[[1]]
Dates_fixed <- as.Date(paste0(Dates, "-01"))  # "YYYY-MM" → Date valida

# Prepara dataframe per tutti i paesi
df_all <- data.frame()

for (cc in countries) {
  ea_index  <- which(dimnames(Y)[[2]] == cc)
  
  gdp_true     <- Y[, ea_index, GDP_index]
  gdp_smoothed <- Y_smoothed[, ea_index, GDP_index]
...
ggplot(df_long_clean, aes(x = Date, y = Value, color = Series)) +
  ...
  labs(
    title = "GDP: True vs Smoothed — All Countries",
    x = "Date", y = "GDP Growth", color = "Series"
  ) +
  theme_minimal(base_size = 13)
